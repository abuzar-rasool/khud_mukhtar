        ...
        onPressed: () async {
          ...
          showDialog<void>(context: context,
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                  title: Icon(
                      Icons.check_circle_outline, size: 90, color: Colors.pink),
                  content: Container(
                    height: 90,
                    child: Column(
                        children: <Widget>[
                          Text('Your service has been added', style:
                          TextStyle(
                            color: Colors.pink,
                          ),),
                          SizedBox(height: 10),
...
class _CustomListViewState extends State<CustomListView> {
  ...
  Future getItems() async {
    ...
    DocumentSnapshot userDoc = await firestore.collection("Users").document(
        userID).get();
    //print(userDoc.data);
    //print(widget.userID);
    if (userDoc.data.containsKey("productList")) {
      ...
      for (String id in myProductIDs) {
        ...
        DocumentSnapshot product = await firestore.collection("Products")
            .document(id)
            .get();
